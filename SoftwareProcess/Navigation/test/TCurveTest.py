...
#        f = def f medthod result
#    outputs:
#       float value .GE. 0
#   Happy path:
#       nominal path: integrate(1) = 
#   Sad path:
#        none as all are prevalidated

        def test500_010_ShouldIntegrateF1(self):
            myT = T.TCurve(self.nominalN)
            self.assertAlmostEquals(myT.integrate(1.0, self.nominalN, myT.f1), 1.0/2, 3)
        def test500_020_ShouldIntegreateF2(self):
            myT = T.TCurve(self.nominalN)
            self.assertAlmostequals(myT.integrate(1.0, self.nominalN,myT.f2), 1.0/3, 3)
        def test500_030_ShouldIntegrateF3(self):
            myT = T.TCurve(self.nominalN)
            self.assertAlmostEquals(myT.integrate(1.0, self.nominalN, myT.f3), 1.0/7, 3)
        def test500_040_ShouldIntegrateF4(self):
            myT = T.TCurve(self.nominalN)
            self.assertAlmostEquals(myT.integrate(1.0, self.nominalN, myT.f4), 1.0/101, 3)
